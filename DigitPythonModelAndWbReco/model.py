...
class Model:
        ...
        for subdir, dirs, files in os.walk(self.data_dir_path, topdown=True):
            ...
            for filename in files:
                ...
                if imghdr.what(filepath) == 'png':
                    ...
                    features.append([name_im])

        spinner.succeed(text='Finished reading')

        print(len(features))
        print(len(labels))
        features = np.array(features, dtype="object").ravel()
        labels = np.array(labels, dtype="object").ravel()
    ...
    def train(self):
        ...
        pipeline_knn_clf.fit(np.ravel(features_train), np.ravel(labels_train))
...
if __name__ == '__main__':
    model = Model(data_dir_path='data/', skip_evaluation=False)
    model.train()
    print ('totot')
    exit(0)
